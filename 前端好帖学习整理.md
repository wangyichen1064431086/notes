...
不会返回相同的东西，第二个函数会返回 undefined。这是由于 Javascript 的分号插入机制决定的，如果某行代码，return 关键词后没有任何东西了，将会自动插入一个分号，显然 foo2 函数中，当 return 后被插入一个封号后，尽管后面的语句不符合规定，但是因为没有执行到，所以也不会报错了。没有 return 任何东西的函数，默认返回 undefined。
...
**坑：**
1. 字符串的solit()方法、数组的reverse()方法都是**就地**执行，即改变了原变量，而非创建一个副本
...
### 16）以上代码可能会由于递归调用导致栈溢出，如何规避这个问题？
